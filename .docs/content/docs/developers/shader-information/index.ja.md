...
Avatar Optimizer v1.9.0-beta.5以降、Avatar Optimizerはあなたのカスタムシェーダーを使用したマテリアルの最適化を支援するShader Information APIを提供しています。
...
- 頂点インデックスがシェーダーで使用されていないと仮定 (`VertexIndexUsage`フラグでオプトアウト可能)
...
Shader Information API全体を通して、`null`値には一貫した意味があります: **不明な値**または**アニメーション化された(静的に決定不可能な)値**を表します。マテリアルプロパティがアニメーション化されている可能性があるか、ビルド時にその値を決定できない場合、APIは不確実性を示すために`null`を返します。
...
シェーダーが使用するテクスチャ、各テクスチャがサンプリングするUVチャンネル、UV変換マトリックス、サンプラーステートに関する情報を提供していることを示します。
...
シェーダーが頂点インデックスを使用し、それを使用する機能が有効になっている場合:
