...
Avatar Optimizer v1.9.0-beta.3以降、Avatar Optimizerはあなたのカスタムシェーダーを使用したマテリアルの最適化を支援するShader Information APIを提供しています。
シェーダー情報を登録することで、Avatar Optimizerがテクスチャアトラス化やUVパッキングなどの高度な最適化を実行できるようになります。
...
現在のAvatar Optimizerはこの情報を使用して以下の方法でアバターを最適化しますが、将来的にはさらに多くの最適化が追加される可能性があります。[^optimization-note]
すべての最適化がTrace and Optimizeで自動的に実行されるわけではないことに注意してください。
...
- **`ShaderInformationRegistry`**: エディタ初期化中に`ShaderInformation`実装をAvatar Optimizerに登録するために使用する静的クラス。
...
Shader Informationはビルド時にのみ使用され、Shader Information APIはEditorビルドでのみ利用可能なため、アセンブリはEditor専用にする必要があります。
...
推奨バージョン範囲: `[1.9-beta.3,2.0)` (v1.9.0-beta.3以降をサポートしますが、v2.0.0では更新が必要になります)。一部のAPIは後のバージョンで追加された可能性があるため、使用するAPIに基づいてバージョン範囲を調整する必要がある場合があることに注意してください。

### 3. Shader Informationクラスを作成 {#create-class}

`ShaderInformation`を継承するクラスを作成し、`ShaderInformationRegistry`に登録してください。
Avatar Optimizerがマテリアルを処理する前に登録されることを保証するため、登録は`InitializeOnLoad`で行う必要があります:
...
`SupportedInformationKind`プロパティは、提供している情報をAvatar Optimizerに伝えます。これはフラグ列挙型なので、`|`演算子で複数の値を組み合わせることができます:
...
このメソッドは頂点インデックスを保持するためのものなので、シェーダーがランダムシーケンスを生成するためだけに頂点インデックスを使用している場合、`RegisterVertexIndexUsage`を登録するのは良い考えではありません。
