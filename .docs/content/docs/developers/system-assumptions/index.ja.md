...
## 外部によるパラメータの使用 {#external-parameter-usage}

パラメータは、AssetDescriptionで明示的に宣言されていない限り、外部ツールによって変更または読み取られないものと仮定します。

Avatar Optimizerは、マークされていない限りパラメータ値が外部から変更または読み取られないという前提に基づいて最適化を行います。ツールが実行時にパラメータを変更または読み取る場合（OSCなど）は、AssetDescriptionを介して外部パラメータとして適切に登録してください。
