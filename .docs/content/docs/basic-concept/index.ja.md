...
BlendShapesを統合すると、そのBlendShapeに対するアニメーションによる動作が変わってしまうため、これはバグとして扱われます。\
...
例えば、`AAO Trace and Optimize`コンポーネントの`BlendShapeを自動的に固定・除去する`は、`AAO Merge Skinned Mesh`コンポーネントによって動くようになる可能性があるBlendShapeであっても、固定・除去します。
...
こういった、振る舞いの維持においてはいくつかの例外があります。
