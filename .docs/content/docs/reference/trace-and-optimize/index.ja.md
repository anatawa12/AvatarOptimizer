...
- `使われていないObjectを自動的に削除する`\
  アニメーションなどを走査して、使われていないObject(GameObjectやコンポーネント、テクスチャなど)を自動的に削除します。\
  また、切り替えるものと一緒に使われていて、他の方法で使われていないPhysBooneコンポーネントを自動的に切り替えるようにします。
  - `endボーンを残す`\
    親が削除されていないendボーン[^endbone]を削除しないようにします。
- `PhysBoneを最適化する`\
  PhysBoneついて、以下の最適化を行います。
  - 全く同じ設定のPhysBone Colliderを1つに統合します。
  - 不要な場合に`Is Animated`のチェックを外します。
  - 掴めないものなど、統合しても動作が変化しない複数のPhysBoneを1つに統合します。
  - Endpoint Positionで問題なく表現できる場合に、endボーンを削除してEndpoint Positionに置き換えます。
- `アニメーターの最適化`\
...
- `スキンメッシュレンダラーを統合する`\
  ...
  - `マテリアルスロットの前後関係を変えることを許可する`\
    ...
    Avatar Optimizerの作者が知る限りにおいて、マテリアルスロットの前後関係は特に意味を持ちません。
- `テクスチャを最適化する`\
