...
internal class BugReportHelper : EditorWindow
{
            ...
            foreach (var component in transform.GetComponents<Component>())
            {
                ...
                void Renderer(Renderer renderer)
                {
                    ...
                    for (var i = 0; i < renderer.sharedMaterials.Length; i++)
                    {
                        ...
                        {
                            builder.AppendLine($"    sharedMaterials[{i}]: {sharedMaterial.name} ({sharedMaterial.shader.name}) ({sharedMaterial.GetEntityIDCompatible()})");
    ...
    private static void RawAnimationInfo(
                     ...
                     .GroupBy(x => x.clip))
        {
            ...
            builder.Append($"  {clip.name}: ({clip.GetEntityIDCompatible()}) (Used in:");
