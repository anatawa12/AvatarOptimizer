...
{
    // TODO: update animations to fix BlendShape animations
            ...
            foreach (var renderer in renderers)
            {
                ...
                for (var i = 0; i < mesh.blendShapeCount; i++)
                {
                    if (mesh.GetBlendShapeFrameCount(i) != 1) {
                        Debug.Log($"BlendShapeCount of {mesh.GetBlendShapeName(i)} of {mesh.name}: {mesh.GetBlendShapeFrameCount(i)}");
                        return false;
                    }
                    // ReSharper disable once CompareOfFloatsByEqualityOperator
                    if (mesh.GetBlendShapeFrameWeight(i, 0) != 100.0f) {
                        Debug.Log($"BlendShapeWeight of {mesh.GetBlendShapeName(i)} of {mesh.name}: {mesh.GetBlendShapeFrameWeight(i, 0)}");
                        return false;
                    }
                }
            }
